...
| Content type | TTL | Examples | Invalidation |
|-------------|-----|---------|-------------|
| Stable | 30 days | Map callouts, agent ability descriptions | Major game update only |
| Semi-stable | 3–7 days | General strategies, lineup guides | New patch marks for refresh |
| Volatile | 4–24 hours | Tier lists, pick/win rates, pro comps | New patch = immediate delete |

Implement TTL checking in the `meta_cache` table: store `expires_at` alongside each entry, query `WHERE expires_at > CURRENT_TIMESTAMP` before using cached content. Run a cleanup job every hour to purge expired rows.
...
# Store with metadata
collection.add(
    documents=["Jett is S-tier on Ascent in patch 12.05..."],
    metadatas=[{"source": "blitz.gg", "type": "volatile", "patch": "12.05"}],
    ids=["blitz_jett_ascent_1205"]
)

# Retrieve with metadata filter
results = collection.query(
    query_texts=["best duelist for Ascent"],
    n_results=5,
    where={"patch": {"$eq": "12.05"}}
)
```
...
### Week 1 — Foundation and core UX validation
...
### Week 2 — Data layer (API + database)

- Implement SQLAlchemy models matching the schema above
- Set up Alembic for migrations
- Build HenrikDev API client with httpx (async, rate-limited with exponential backoff)
- Implement match sync pipeline: `valocoach sync` fetches recent matches, upserts into SQLite
- Implement `valocoach profile` from `v_player_profile` view
...
- Sample-size threshold warnings
- **Testing**: compare computed ACS against API-reported `score/roundsPlayed` for 10+ matches — assert equality within ±1 (rounding)
...
- Implement Trafilatura + BS4 extraction chain
- Set up `youtube-transcript-api` for coaching channel transcripts
- Implement chunking (512 tokens, 50 overlap, paragraph-boundary-aware)
- Set up nomic-embed-text via Ollama (`ollama pull nomic-embed-text`)
- Initialize ChromaDB with collections: `patch_notes`, `strategies`, `pro_meta`, `coaching_content`
- Implement retrieval pipeline: query → embed → search → rank → return top-k
- Implement patch detection via HenrikDev `/version` endpoint
- Build TTL-based cache invalidation
- Implement `valocoach meta` command

**Deliverable**: RAG retrieves relevant content for any coaching query. `valocoach meta --map Haven` returns current meta info.

### Week 5 — LLM coaching pipeline

- Implement system prompt with all grounding rules
- Build coaching orchestrator: parse input → fetch player stats → retrieve RAG context → compose prompt → stream response
- Implement input parsing (regex first, full text to LLM)
- Build adaptive context trimming (priority-based token budget)
- Implement interactive mode with prompt_toolkit: autocomplete, history, slash commands
- Implement conversation memory with sliding window + summarization
- Iterate on prompt engineering: test 20+ scenarios, refine system prompt

**Deliverable**: Full end-to-end coaching. `valocoach interactive` supports multi-turn conversations.
...
│   │   └── completers.py     # prompt_toolkit completers
│   ├── core/
│   │   ├── config.py         # pydantic-settings
│   │   ├── models.py         # Pydantic data models
│   │   └── exceptions.py
│   ├── data/
│   │   ├── api_client.py     # HenrikDev API (httpx)
│   │   ├── database.py       # SQLAlchemy models + session
│   │   ├── migrations/       # Alembic
│   │   └── sync.py           # Match sync orchestration
│   ├── stats/
│   │   ├── calculator.py     # Core stat formulas
│   │   ├── analyzer.py       # Trends, weaknesses, z-scores
│   │   └── formatters.py     # Stat display formatting
│   ├── retrieval/
│   │   ├── scraper.py        # Trafilatura + BS4
...
│       ├── coach.py          # Coaching pipeline orchestrator
